...
while (capture.isOpened()):
    ...
    #rotation 180
    M = cv2.getRotationMatrix2D((w_r/4, h_r/4), 180, 1)
    dst = cv2.warpAffine(gray, M, (w_r, h_r))
    M_nozzle= cv2.getRotationMatrix2D((w / 2, h / 2), 180, 1)
    dst_nozzle = cv2.warpAffine(nozzle,M_nozzle,(w,h))

    # 每一幀都換一次template

    if flag_temp == 1:
        res = cv2.matchTemplate(gray, nozzle, cv2.TM_CCOEFF)
        res_r = cv2.matchTemplate(dst, dst_nozzle, cv2.TM_CCOEFF)
        flag_temp = 0
        gray_new = gray
        dst_new = dst
    else:
        new_temolate=gray_new[left_top[1]:left_top[1]+h,left_top[0]:left_top[0]+w ]
        new_template_r=dst_new[ max_loc_r[1]:max_loc_r[1]+h,max_loc_r[0]:max_loc_r[0]+w]
        #cv2.imshow("template",new_temolate)
        res = cv2.matchTemplate(gray, new_temolate, cv2.TM_CCOEFF)
        res_r = cv2.matchTemplate(dst, new_template_r, cv2.TM_CCOEFF)
        gray_new=gray
        dst_new=dst
    ...
    right_bottom=(360-max_loc_r[0],640-max_loc_r[1])
    #cv2.imshow("ddd",dst_nozzle)
   # print(left_top)
    #print(max_loc_r)
    #print(right_bottom)
    ...
    roiMaxX = 500
    roiMaxY = 570
    roiMinY = (int)((2 * left_top[1] + h) / 2 + h / 2 )

    roi = temp[roiMinY:roiMaxY, roiMinX:roiMaxX]

    if abs(roiMinY - buzzle) > 30 and buzzle != 0:
        print("jump")
        continue
    ...
    labelnum, labelimg, contours, GoCs = cv2.connectedComponentsWithStats(sure_fg, 8, cv2.CV_32S)
